...
- **Usernames and addresses.** Everyone who makes apps has a unique username (chosen at sign-up, changeable in My page → Profile). jhino.com/<username> is their public page; their app addresses and short links live under it: jhino.com/<username>/<name>. Only super admins give top-level addresses (jhino.com/<name>), which can never be a username. Addresses made before usernames keep working at the top.
- **My page (link in bio).** At jhino.com/<username>: links, headings, text, YouTube and Vimeo videos, the person's Jhino apps, and social icons (Instagram, Facebook, TikTok, YouTube, WhatsApp, Viber and more, from a handle or number). Two layouts (a list of links, or a profile) and 30 designs: Free 5, Plus 15, Pro all 30, and Pro can replace the design with their own HTML (served sandboxed at /p/<username>/custom with `{{name}}`, `{{links}}` and friends). Free pages carry a Jhino badge and popup, Plus a small badge, Pro nothing. Every link goes through /go/<id>, so clicks are counted; views, visitors (once a day, from a daily-salted hash), referrers, devices and countries (from Cloudflare's CF-IPCountry) show in My page → Analytics, 7 days on Free, 30 on Plus, a year on Pro.
- **Email codes.** Confirming an email, resetting a password and changing an email each send a 6-digit code. A code works for 5 minutes, once. Asked for again within those 5 minutes, the same code is sent again (emails sometimes arrive late), up to 5 times and at least 20 seconds apart. Five wrong tries lock a code; per account there are at most 30 codes sent and 20 wrong codes a day, whatever address they come from. Codes are derived from a key in `DATA_DIR/system/codes.key` (outside the database), so only hashes are stored, and the email log keeps them masked once email is delivered.
- **Confirmed email.** New sign-ups get no session until they enter the code (signing in before that re-sends the code). Customers who signed up earlier and never confirmed see a "Confirm your email" window over the dashboard until they do, with a way to change to a real address first; their apps and data are untouched. Accounts a super admin makes count as confirmed; a studio's client accounts are not asked.
- **Usernames people choose** stay off general words (faq, services, about-us, pricing, work, tasks, contact…, in any dressing: `our-services`, `servicespage`, `faqs`, `services2`), brand and payment names, and anything that reads as Jhino or its staff (`server/reserved.ts`). A super admin can give any name that is free (general words, "jhino", 2 to 50 characters) at any time, with no 30-day wait; the only names nobody can have are Jhino's own addresses (`SYSTEM_PATHS` in `server/publicshare.ts`: login, apps, admin, api…), because those pages answer first. People change their own username once in 30 days; the old one is free for anyone at once.
- **Two-step sign-in** (Account → Security): a code from an authenticator app after the password (or Google/Apple), with 8 single-use recovery codes. The secret is stored encrypted. It is optional for everyone; super admin sessions end after 3 days.
- **Apps made** (Super Admin → Apps made): every app people create and how (Create app, HTML or ZIP upload), builds and updates by day, who makes the most, and the latest apps.
- **Analytics** (Super Admin → Analytics): how many people are on Jhino right now and what they have open (the website, dashboards, public pages, any customer's app), a live feed, visitors and views by day and by hour, the apps getting traffic, countries, referrers, pages, devices and browsers. Visitors are a daily-changing salted hash; no addresses are stored; bots are left out.
- **Videos are links.** Customers share videos as YouTube, Vimeo, Google Drive or Dropbox links (Create app's video deliveries play them in the page); video files are refused on upload. Super admins can still upload them, and big ones are made smaller.
...
**Automatic:** while it runs, Jhino copies the database once a day to `BACKUP_DIR/auto/jhino-<date>.db` and keeps the last 14 (`AUTO_BACKUP=0` turns it off). The database holds every account, app record, payment and setting. To restore one: stop Jhino, copy the snapshot over `DATA_DIR/jhino.db`, delete `jhino.db-wal` and `jhino.db-shm` next to it, start Jhino.

**Full (database, apps and files):**
...
- Behind Cloudflare, the visitor's address is taken only from hops Jhino trusts (local proxies and Cloudflare's published ranges, plus `TRUSTED_PROXIES`), so rate limits cannot be dodged with a forged `X-Forwarded-For`. Sign-in misses back off per account, whatever the address. Logged request addresses drop query strings and link tokens.
